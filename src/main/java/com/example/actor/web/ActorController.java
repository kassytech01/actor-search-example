...
@Controller
@SessionAttributes(types = { ActorForm.class })
public class ActorController {
	...
	@RequestMapping(value = "/actor/create_prev", method = RequestMethod.GET)
	public String create_prev(ActorForm form, Model model, SessionStatus sessionStatus) {
		logger.debug("Actor + create_prev");
	...
	@RequestMapping(value = "/actor/confirm", method = RequestMethod.POST)
	public String confirm(@Validated @ModelAttribute ActorForm form, BindingResult result, Model model) {
		logger.debug("Actor + confirm");
		if (result.hasErrors()) {
			String message = msg.getMessage("actor.validation.error", null, Locale.JAPAN);
			model.addAttribute("errorMessage", message);
			return create(form, model);
		}
		if (form.getBirthplaceId() != null) {
			Prefecture pref = prefectureRepository.findOne(Integer.parseInt(form.getBirthplaceId()));
			model.addAttribute("pref", pref);
		}
		return "Actor/confirm";
	}

	@RequestMapping(value = "/actor/save", method = RequestMethod.GET)
